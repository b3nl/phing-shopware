    ...
    {else}
        ...
        {block name='frontend_account_password_form'}
            <form name="frmRegister" method="post" action="{url action=password}">
                <h2>{se name="PasswordHeader"}{/se}</h2>
                <div class="form col two">
                    ...
                    <p class="buttons">
                        <a href="javascript:history.back();" class="btn neutral">{se name="PasswordLinkBack"}{/se}</a>
                        <input type="submit" class="btn" value="{s name="PasswordSendAction"}Passwort anfordern{/s}" />
                    </p>
                </div>
            </form>
        {/block}
