...
<div id="confirm" class="">
    ...
    <div class="personal-information row">
        ...
        <div class="row">
            ...
            {block name='frontend_checkout_confirm_left_billing_address'}
                <div class="invoice-address col four">
                    <h3>{s name="ConfirmHeaderBilling" namespace="frontend/checkout/confirm_left"}{/s}</h3>
                    ...
                    {* Action buttons *}
                    <div class="buttons">
                        <a href="{url controller=account action=billing sTarget=checkout}" class="btn neutral">
                            {s name="ConfirmLinkChangeBilling" namespace="frontend/checkout/confirm_left"}{/s}
                        </a>
                        <a href="{url controller=account action=selectBilling sTarget=checkout}" class="btn neutral">
            ...
            {block name='frontend_checkout_confirm_left_shipping_address'}
                <div class="shipping-address col four">
                    ...
                    {* Action buttons *}
                    <div class="buttons">
                        <a href="{url controller=account action=shipping sTarget=checkout}" class="btn neutral">
                            {s name="ConfirmLinkChangeShipping" namespace="frontend/checkout/confirm_left"}{/s}
                        </a>

                        <a href="{url controller=account action=selectShipping sTarget=checkout}" class="btn neutral">
            ...
            {block name='frontend_checkout_confirm_left_payment_method'}
                {if !$sRegisterFinished}
                    <div class="payment-display col four">
                        ...
                        {* Action buttons *}
                        <div class="buttons">
                            <a href="{url controller=account action=payment sTarget=checkout}" class="btn neutral">
    ...
    {if {config name=commentvoucherarticle}||{config name=premiumarticles}||{config name=bonussystem} && {config name=bonus_system_active} && {config name=displaySlider}}
        <div class="additional-options row">
            ...
            <h2>{s name="ConfirmHeadlineAdditionalOptions"}Weitere Optionen{/s}</h2>
            <div class="inner_container">
                <div class="row">
                    {* Voucher and add article *}
                    {if {config name=commentvoucherarticle}}
                        <div class="voucher-add-article col two">
                            ...
                            {block name='frontend_checkout_table_footer_left_add_voucher'}
                                <div class="vouchers form">
                                    <form method="post" action="{url action='addVoucher' sTargetAction=$sTargetAction}">
                                        ...
                                        <input type="submit" value="{s name='CheckoutFooterActionAddVoucher' namespace="frontend/checkout/cart_footer_left"}{/s}" class="btn" />
                                    </form>
                                </div>
                            {/block}
                            ...
                            {block name='frontend_checkout_table_footer_left_add_article'}
                                <div class="add_article form">
                                    <form method="post" action="{url action='addArticle' sTargetAction=$sTargetAction}">
                                        ...
                                        <input type="submit" class="btn" value="{s name='CheckoutFooterActionAdd' namespace="frontend/checkout/cart_footer_left"}{/s}" />
                                    </form>
                                </div>
                            {/block}
                        </div>
                        ...
                        {block name='frontend_checkout_confirm_comment'}
                            <div class="form user-comment col two">
                    ...
                    {block name='frontend_checkout_confirm_submit'}
                        ...
                        <div class="actions">
                            {if $sPayment.embediframe || $sPayment.action}
                                <input type="submit" class="btn checkout" id="basketButton" value="{s name='ConfirmDoPayment'}Zahlung durchführen{/s}" />
                            {else}
                                <input type="submit" class="btn checkout" id="basketButton" value="{s name='ConfirmActionSubmit'}{/s}" />
                            {/if}
                        </div>
                    {/block}
                {else}
