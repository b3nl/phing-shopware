...
<div id="center" class="register change_payment">
    <div class="inner">
        ...
        <div class="form">
            ...
            <form name="frmRegister" method="post" action="{url action=savePayment sTarget=$sTarget}" class="payment">
                ...
                {block name="frontend_account_payment_action_buttons"}
                    {if $sTarget}
                        <a class="btn neutral" href="{url controller=$sTarget}" title="{s name='PaymentLinkBack'}{/s}">
                            {se name="PaymentLinkBack"}{/se}
                        </a>
                    {/if}
                    <input type="submit" value="{s name='PaymentLinkSend'}{/s}" class="btn" />
                {/block}
            </form>
        </div>
    </div>
