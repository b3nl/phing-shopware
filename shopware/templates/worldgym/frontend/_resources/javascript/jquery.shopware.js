...
//    Changing article informations on variants
    ...
    $('.account .change_password').on('click', function (event) {
        event.preventDefault();
        var self = $(this);

        $('.account .password').toggle().toggleClass('active');
        $('.account .email').hide();

        $('html, body').animate({scrollTop:self.offset().top + 40});
    });

    //Change email account
    $('.account .change_mail').on('click', function (event) {
        event.preventDefault();
        var self = $(this);

        $('.account .email').toggle().toggleClass('active');
        $('.account .password').hide();
        $('html, body').animate({scrollTop:self.offset().top + 40});
...
jQuery.fn.liveSearch = function (conf) {
    ...
    if (!liveSearch.length) {
        liveSearch = jQuery('<div id="' + config.id + '"></div>').appendTo('#searchcontainer').hide().slideUp(0);
...
(function ($) {
    ...
    $.modalFrame = function (url, headline, settings) {
        ...
        var close = $('<a>', {
            'html': '<p>Fenster schließen</p><span data-icon="&#xf00d;" class="icon" aria-hidden="true"></span>',
            'class': 'modal_close btn_close'
        }).appendTo(modal);
        ...
        $('a.buynow').live('click', function (event) {
            ...
            $.ajax({
                ...
                'success': function (result) {
                    ...
                    $('.article-slider').slick(slickSliderConfig);
...
(function ($) {
    ...
    function CloudZoom(jWin, opts) {
        ...
        this.init = function () {
            ...
            // lets make the mousetrap responsive
            var mouseTrap           = $('.mousetrap'),
                zoomImg             = $('#zoom1 img'),
                zoomImageWidth      = 0,
                zoomImageHeight      = 0;

            var recalcMousetrap = function(){
                    zoomImageWidth  = zoomImg.width(),
                    zoomImageHeight = zoomImg.height();
                //console.log(zoomImageHeight);

                mouseTrap.height(zoomImageHeight);
                mouseTrap.width(zoomImageWidth);
            }
            ...
            $mouseTrap.bind('mouseenter', this, function (event) {
                ...
                zoomDiv = $('#detail').append($.format('<div id="cloud-zoom-big" class="cloud-zoom-big" style="display:none;position:absolute;left:%0px;top:%1px;width:%2px;height:%3px;background:#FFF url(\'%4\') no-repeat;z-index:99;"></div>', pos.left + sImg.width() + 15, pos.top - 4, w, h, zoomImage.src)).find(':last');
...
;(function ( $, window, document, undefined ) {
    ...
    var createTemplate = function(article, lastClass) {
        var rule, link, image, hidden, desc;
