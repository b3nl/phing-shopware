<div class="table_row{if $lastitem} lastrow{/if} row">
	...
	<div class="col textright">
		<div class="textright">
			<strong>
			<a href="#order{$offerPosition.ordernumber}" title="{s name='OrderActionSlide'}{/s} {$offerPosition.ordernumber}" class="orderdetails btn" rel="order{$offerPosition.ordernumber}">
...
<div id="order{$offerPosition.ordernumber}" class="displaynone row">
	<div class="table">
		...
		<div class="table_foot">
			...
			<form method="post" action="{url controller='checkout' action='add_accessories'}">
				...
				{if $offerPosition.activeBuyButton}
					<input type="submit" class="btn checkout" value="{s name='OrderLinkRepeat'}{/s}" />
				{/if}
			</form>
			{/block}
			
			{if $offerPosition.customercomment}
				<h4>{se name="OrderItemCustomerComment"}Ihr Kommentar{/se}</h4>
				<blockquote>
			...
			{if $offerPosition.comment}
				<h4>{se name="OrderItemComment"}Unser Kommentar{/se}</h4>
				<blockquote>
